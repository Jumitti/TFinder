...
with st.sidebar.expander("Promoter Finder"):
    ...
    st.sidebar.write("There is no limit to the number of gene names/ENTREZ_GENE_ID. Add them with a line break (like those displayed by default). You can mix ENTREZ_GENE_ID and gene names as long as they are of the same species.")
    ...
    st.sidebar.write('FASTA format: All sequences must have the TSS at the same distance, otherwise you assume the inconsistency of the positions of found sequences')
st.sidebar.divider()
with st.sidebar.expander("Responsive Elements Finder"):
    st.sidebar.subheader("Responsive element:")
    st.sidebar.write('IUPAC allowed')
    st.sidebar.image("https://raw.githubusercontent.com/Jumitti/Responsive-Elements-Finder/main/img/IUPAC.png")
    st.sidebar.subheader("Transcription Start Site (TSS):")
    st.sidebar.write('Distance to Transcription Start Site (TSS) in bp')
    st.sidebar.write('Note: If you use "Promoter finder", it will be defined automatically.')
    st.sidebar.subheader("Threshold:")
    st.sidebar.write('Eliminates responsive element with homology < threshold')
    st.sidebar.write('Note: Homology is calculated between the responsive element in the promoter and the responsive element requested. The calculation uses the Hamming distance, counts the number of differences and gives a percentage score.')
