...
# Upstream/Downstream
label_visibility  = [(-10000, -1, "Upstream"), (0, "TSS"), (1, 10000, "Downstream")]
updown_slide = st.slider("Upstream/downstream from the TSS (bp)", -10000, 10000, (-5000, 5000), step=100, format="%d", key="updown_slider", label_visibility =label_visibility)
...
with st.sidebar.expander("Responsive Elements Finder"):
    ...
    st.write('Distance to Transcription Start Site (TSS) in bp')
    st.write('Note: If you use "Promoter finder", it will be defined automatically.')
    st.subheader("Threshold:")
    st.write('Eliminates responsive element with homology < threshold')
    st.write('Note: Homology is calculated between the responsive element in the promoter and the responsive element requested. The calculation uses the Hamming distance, counts the number of differences and gives a percentage score.')
